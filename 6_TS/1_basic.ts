/*
    1. 타입주석
     ...
     - 타입주석을 통해 정적 타입 검사가 가능해지며, 잘못된 타입의
       값이 들어갈 경우 컴파일 에러를 발생시킨다.
*/
let num :number;
num = 12345;
// num = '12345';
...
/*
    2. 타입추론
     ...
     - 간단한 변수 선언시에는 타입주석을 작성하지 않고 타입추론을
       활용하는 것이 좋다.
     - 단, 복잡한 함수의 매개변수나 반환형, api명세등 명확한 타입정의가
       필요한 곳에는 반드시 작성한다.
*/
let userName = "KH 정보교육원";
let age = 5;
// userName = 123; // 'number' 형식은 'string' 형식에 할당할 수 없습니다.

/*
    3. 배열과 튜플

    배열의 특징
     - 배열의 크기에 대한 제약이 존재하지 않으며, 들어가는 타입에 대한 제한도 없다.
...
/*
    4. readonly
     ...
     - 단, 튜플은 js에는 존재하지 않는 타입으로 컴파일 완료 후에는 배열로 변환된다.
     - 타입스크립트의 정적타입체크 시스템은 실제 코드를 실행하기 전 단계에서 작동하므로
       실행환경에서 동작하는 메서드에 대한 검사는 수행하지 않는다.
*/
numTuple.push(1111);
console.log(numTuple); // 튜플의 길이고정 규칙이 깨짐
// console.log(numTuple[3]); [1,2,3,1111] 값이 있으나 오류발생
...
//  - 단순히 값을 받기위한 용도로만 사용하며, unknown에 할당된 데이터를
//    사용하기 위해서는 적절한 타입으로의 narrowing필요
// str = unknown;
// num2 = unknown;
