...
/*
    4. restParameter
     - 함수의 매개변수에 들어갈 인자의 수가 정해지지 않은 경우
       사용하는 ES6문법
...
restParameter(...spreadArr); // [1,2,3,4,5]

function normalFn(a:number, b:number){
    console.log(a, b);
}
/*
    배열은 크기가 정해지지 않은 타입이기 때문에, 매개변수의 갯수가
    고정된 일반함수에서 전개연산자를 통해 인자를 전달할 수 없다.
    -> 자릿수가 정해진 튜플로 해결가능.
*/
// normalFn(1, 2, ...spreadArr);
...
/*
    7. 타입 좁히기(Type narrowing)
     - unionType이나 unknown처럼 변수가 여러타입을 가지고 있을 때,
       이를 사용하는 시점에서 특정 한가지로 좁히는 문법
...
/*
    8. Type Assertion
     - 여러타입을 가질 수 있는 변수에 대하여 "개발자가 직접 " 해당 값의 타입을 명확히 지정하는 문법.
...
console.log(typeAssertion('mkm'));
console.log(typeAssertion(123)+123); // string + 123 ??

// not null 단언문
//  - 선택한 변수의 값이 null(undefined 포함)이 아님을 단언하는 문법
function notNullAsserrion(number?:number) :number{
    return number!;
}

/*
    9. never type
     - 어떤 값도 가질 수 없는 타입을 의미하며, 절대 값을 반환할 수 없는 함수나,
