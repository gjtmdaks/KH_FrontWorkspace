...
const print:PrintType = (name, favorite) => {
    console.log(`안녕하세요 . 제 이름은 ${name}입니다. 제 최애 음식은 ${favorite}입니다, 하나만 사주세요`);
}
print('mkm','돈까스');
...
function racoonInfo(name:string, weight:number, gender:string, d?:boolean) :string {
    if(d != undefined){
        return `이름 : ${name}, 무게 : ${weight}, 성별 : ${gender}, 중성화 : ${d}`;
    }
    return `이름 : ${name}, 무게 : ${weight}, 성별 : ${gender}`;

    return `이름 : ${name}, 무게 : ${weight}, 성별 : ${gender} ${d != undefined ? `, 중성화 ${d}` : ``}`;
}

// Q4. sum함수의 몸통부분을 완성하시오
...
// Q6. 매개변수로 들어온 모든 숫자를 곱하여 반환하는 함수 작성
// multiplyAll의 매개변수는 1개 이상의 매개인자가 들어올 수 있습니다.
...
function assertNever(value :never) :never{
    throw new Error(`타입가드를 위한 보조함수 작동 ${value}`);
}
const any:any = [1,2,3];
handleValue('1');
handleValue(1);
...
*/
type FnType = ([first, ...rest] :[number, ...number[]]) => void;
const fn:FnType  = ([first, ...rest]) => {
    console.log(rest.map(i => i+first));
}
// fn([]) // 컴파일에러
fn([1]); // []
fn([1,2]); // [3]
