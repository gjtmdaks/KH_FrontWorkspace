...
const arr2 = defValue(45); // 타입추론에 의해 number[] / 언제나 타입추론이 먼저 동작
const arr3 = defValue(); // 기본값사용. / 매개변수가 없다면 기본타입으로 설정
// 기본값 미설정 시 unknown
...
function config<T extends Options = {theme:'dark'}>(options?:T) :T{
    ...
    // 널병합연산자 ??
    //  - a ?? b-> a가 null 또는 undefined가 아니라면 A,
    //    맞다면 B 반환
    // 삼항연산자의 단축키
    return options ?? {theme:'dark'} as T;
}
...
/*
    3) Omit<T, K>
     - 객체타입에서 특정 속성을 제외한 나머지만 남긴 채 타입을 만들때 사용
*/
...
//  - 같은 속성이 병합되는 경우, 좀 더 구체적인 속성으로 병합
