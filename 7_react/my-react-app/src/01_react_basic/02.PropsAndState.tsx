...
/*
    #1. Props
     ...
     - 자식컴포넌트는 전달받은 props를 읽기전용으로 사용해야 한다.
...
export default function ParentComponent(){
    // 자식에게 전달한 속성과 데이터 준비
    const style:React.CSSProperties = {color:'green', background:'black'};
    ...
    /*
        #2. State
         ...
         - 부모컴포넌트에서 여러 자식컴포넌트에 공유해야할 데이터가 있는
           경우 부모컴포넌트에서 state를 선언 후 자식에게 props로 전달해야한다.
    */
...
function ChildComponent({style, message, setMessage}:ChildProps){
    ...
    return (
        <div>
            ...
            <button onClick={messageHander}>messageHender</button>
