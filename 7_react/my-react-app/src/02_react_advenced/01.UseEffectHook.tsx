...
export default function UseEffectHook(){
    /*
        #1. Hook
         - Hook은 함수형 컴포넌트 내부에 React의 기능(state 선언 및 관리,
           생명주기 함수)을 연결할 수 있도록 도와주는 함수.
         - 원래 React에서는 컴포넌트가 복잡한 동작을 수행하려면 클래스 컴포넌트를
           써야했지만, 그럴 경우 코드가 복잡하고 가독성이 떨어졌음.
    ...
    /*
        #2. useEffect
         - React컴포넌트가 실행되고(mount), 삭제되고(unmount),
           업데이터(update)될 때를 감지하여 특정 작업을 수행할 수 있게
           도와주는 hook 함수
    ...
    */
    ...
    // api서버와 통신하여 초기 데이털ㄹ load하고자 할때
    useEffect(() => {
        console.log("서버와 통신");
