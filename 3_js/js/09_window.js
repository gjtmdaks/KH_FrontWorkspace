/* 
    window
     - 웹브라우저에 의해 탭마다 하나씩 생성되는 객체로 브라우저를
       제어할 수 있는 메서드를 제공한다.
     - 웹브라우저상의 취상위 객체로, javascript를 실행하는 환경에 따라
       존재하지 않을 수도 있다.
...
function test3(){
    /* 
        setTimeout(callback, millis)
         ...
         - timeoutApi의 메서드 중 하나로, timeoutApi는 비동기적으로
           처리된다.
...
function test4(){
    var i = 1;
    console.log(1,i++); // 1번 1,1(2)
    test(i); // 2번 2가 넘어감 2(2)
    setTimeout(function(){
        console.log(2, i++); // 4번 2,2(3)
    }, 1000);
    setTimeout(function(){
        console.log(4, i++); // 5번 4,3(2)?
    }, 1000);
    console.log(3,i); // 3번 3,2
}

function test(i){ // 2번 2가 넘어옴
    console.log(i--); // 2(1) 1값은 넘어가지 않음
}
...
function test7(){
    ...
    var a = setInterval(function(){
        ...
        num == 0 && (countdown.innerHTML = "뻥~") && (clearInterval(a));
        // if(num<0){
...
function test9(){
    ...
    */
    ...
    // location.replace("https://naver.com"); // url자체를 교체(뒤로가기 불가능)
    // location.href = "https://naver.com"; // 보통 잘 안쓰임
}
...
function test11(){
    ...
    */
    ...
    var top = (screen.availHeight-h)/2 + screen.availTop;
