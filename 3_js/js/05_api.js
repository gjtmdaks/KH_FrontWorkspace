test1.onclick = function(){
    ...
    console.log(str.substring(2,4)); // 2,3번째 인덱스값 추출
    console.log(str.substring(2)); // 2번 인덱스부터 끝까지
...
test2.onclick = function(){
    ...
    console.log(Math.ceil(num)); // 무조건 올림
    console.log(Math.floor(num)); // 무조건 버림
...
test4.onclick = function(){
    ...
    console.log(7 % Number("4")); // 3, number
    ...
    console.log(7 + true); // 8, true는 number로 변환시 1로 처리
    console.log(7 + false); // 7, false는 0

    // 모든 연산시에는 원시값과 원시값끼리만 연산이 가능하기 때문에, 참조변수는 원시값으로
    // 변환된 후 연산처리 된다.
    console.log([] + 1); // 1, String
    // [] -> [].toString() = '' + 1 -> '' + '1' => '1'
    // [5] -> [5].toString() = '5' + 1 -> '5' + '1' => '51'

    console.log({} + 1); // {} -> [object Object]

    function fn(){};

    console.log(fn + 1); // fn -> function fn(){}
};

test5.onclick = function(){
    var num = "1234.567원";
    console.log(Number(num)); // NaN '원'때문에
...
test7.onclick = function(){
    ...
    */
    ...
    console.log(Boolean([]+'')); // false [].toString으로 인해 ''빈문자열 반환
    console.log(Boolean({})); // true
